          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            {weekDays.map((dayInfo, index) => (
              <Tooltip key={index}>
                    ...
                    <CardContent className="p-4 text-center space-y-2 relative">
                      ...
                      {dayInfo.hasSession ? (
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <Badge
                            variant="outline"
                            className="text-xs px-2 py-0.5 border-[#3b82f6] text-[#3b82f6]"
                          >
                            Planned
                          </Badge>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <Badge
                            variant="outline"
                            className="text-xs px-2 py-0.5 border-[#6b7280] text-[#6b7280]"
                          >
                            Rest
                          </Badge>
                        </div>
                ...
                {/* ✅ Tooltip hiển thị exercises */}
                {dayInfo.exercises.length > 0 && (
                  <TooltipContent
                    side="top"
                    className="bg-white border border-[#e5e7eb] rounded-lg p-3 max-w-xs"
                  >
                    <div className="space-y-1">
                      <p className="text-xs font-semibold text-[#111827]">
                        {dayInfo.day}'s Workout
                      </p>
                      <ul className="text-xs text-[#6b7280] space-y-0.5">
                        {dayInfo.exercises.slice(0, 3).map((exercise, i) => (
                          <li key={i}>• {exercise}</li>
                        ))}
                        {dayInfo.exercises.length > 3 && (
                          <li className="italic">
                            +{dayInfo.exercises.length - 3} more
                          </li>
                        )}
                      </ul>
                    </div>
