...
export function WorkoutsCard({ thisWeekCount, lastWeekCount, weeklyGoal }: WorkoutsCardProps) {
    ...
    <Card className="rounded-2xl border-[#e5e7eb] shadow-sm bg-white">
      ...
      <CardContent className="space-y-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-sm ${difference >= 0 ? 'text-[#10b981]' : 'text-[#ef4444]'}`}>
        ...
        <div className="space-y-2">
          <div className="flex justify-between text-xs text-[#6b7280]">
            ...
            <span>{thisWeekCount}/{weeklyGoal}</span>
          </div>
          <Progress 
