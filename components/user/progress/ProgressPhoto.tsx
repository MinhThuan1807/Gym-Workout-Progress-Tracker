        ...
        <Card className="rounded-2xl border-[#e5e7eb] shadow-sm bg-white">
          <CardContent className="p-12 text-center">
            ...
            <p className="text-[#6b7280] mb-6">
              Start your transformation journey by uploading your first photo
            </p>
            <Button
              className="rounded-xl bg-[#10b981] hover:bg-[#059669]"
              onClick={() => setIsUploadModalOpen(true)}
            ...
            <div className="space-y-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    value={uploadView}
                    onValueChange={(value) => setUploadView(value as PhotoView)}
                  >
                    <SelectTrigger className="rounded-xl border-[#e5e7eb]">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {(['front', 'back', 'side'] as PhotoView[]).map(
                        (view) => (
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex gap-1 bg-[#e5e7eb]/50 p-1 rounded-xl">
              <Button
                ...
                className={`rounded-lg h-8 ${
                  viewMode === 'grid' ? 'bg-[#3b82f6] hover:bg-[#2563eb]' : ''
                }`}
              >
      ...
      {viewMode === 'timeline' && (
        <div className="space-y-6">
          {photosByDate.map(([date, datePhotos]) => (
            <Card
              key={date}
              className="rounded-2xl border-[#e5e7eb] shadow-sm bg-white"
            >
              <CardHeader>
      ...
      {selectedPhotoForView && (
        <Dialog
          open={!!selectedPhotoForView}
          onOpenChange={() => setSelectedPhotoForView(null)}
        >
            ...
            <DialogHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-[#6b7280]">
                    {new Date(selectedPhotoForView.takenAt).toLocaleDateString(
                      'en-US',
                      {
                        weekday: 'long',
                        month: 'long',
                ...
                <Badge className="bg-[#10b981] text-white rounded-lg">
                  {viewIcons[selectedPhotoForView.view]}{' '}
                  {selectedPhotoForView.view}
                </Badge>
              </div>
            </DialogHeader>
                ...
                <Button
                  ...
                  onClick={() => {
                    const currentIndex = filteredPhotos.findIndex(
                      (p) => p._id === selectedPhotoForView._id
                    )
                    if (currentIndex > 0) {
